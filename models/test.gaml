...
global { //chargement des fichiers shapes
	...
	int nb_personne parameter:  'Nombre de personnes' init: 50 min:20 category:"LES VERSIONS DE SIMULATION";
	int capacite_ascenseur parameter: 'Capacite ascenseur ' init: 5 min:1 category:"LES VERSIONS DE SIMULATION";
	float Probabilite_Nier parameter: 'Probabilite nier ' init: 0.1 min:0.1 category:"LES VERSIONS DE SIMULATION";
	float Probabilite_patienter parameter: 'Probabilite nier ' init: 0.1 min:0.1 category:"LES VERSIONS DE SIMULATION";
	...
	reflex aller_au_but {
		if (est_occuper) {
			if (self distance_to sa_destination.location <= 3) {
				est_arriver <- true;
				if (position = "monter") {
					Personnes <- list(personne) where ((each.peut_embarquer = true) and (each distance_to self <= 180) and !deja_recuperer);
					if (Personnes != []) {
...
species feu {
	...
	reflex propagation_feu when: etat = "actif" {
...
species personne skills: [moving] {
	...
	//list les_zones <- list(zone);
		...
		if (etat = "evacuation") {
			draw circle(10) color: #yellow depth: 0.1;
		}
...
//        data "numberA" value: 40 color: #red;
//        data "numberB" value: 60 color: #blue;
//        }
//        }
